...
class NuScenesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if p.exists():
            ...
            # Fallback: generate mask on-the-fly using nuScenes devkit
            canvas_hw = (800, 800)
            ego_pose = self.nusc.get(
                'ego_pose',
                self.nusc.get('sample_data', d['token'])['ego_pose_token']
            )
            yaw_deg = self._quat_to_yaw_deg(ego_pose['rotation'])   # now works!
            m = self._render_map_mask(                             # now works!
                location=d['location'],
